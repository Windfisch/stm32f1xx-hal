...
use crate::dma::dma2;
use crate::dma::{Transfer, TransferPayload, Receive, Transmit, RxDma, RxTxDma, TxDma, R, W};
...
pub type SpiRxTxDma<SPI, REMAP, PINS, RXCHANNEL, TXCHANNEL> = RxTxDma<SpiPayload<SPI, REMAP, PINS>, RXCHANNEL, TXCHANNEL>;
...
macro_rules! spi_dma {
    ($SPIi:ident, $RCi:ty, $TCi:ty) => {
        ...
        impl<REMAP, PINS> Spi<$SPIi, REMAP, PINS, u8> {
            ...
            pub fn with_rx_tx_dma(self, rxchannel: $RCi, txchannel: $TCi) -> SpiRxTxDma<$SPIi, REMAP, PINS, $RCi, $TCi> {
                let payload = SpiPayload { spi: self };
                SpiRxTxDma { payload, rxchannel, txchannel }
            }
        }
        ...
        // FIXME TODO: what to do about the R/W flag in transfer? seems to only control peek()? should it be possible to peek() it?


        impl<RXB, TXB, REMAP, PIN> crate::dma::ReadWriteDma<RXB, TXB, u8> for SpiRxTxDma<$SPIi, REMAP, PIN, $RCi, $TCi>
        where
            RXB: StaticWriteBuffer<Word = u8>,
            TXB: StaticReadBuffer<Word = u8>,
        {
            fn read_write(mut self, mut rxbuffer: RXB, mut txbuffer: TXB) -> Transfer<R /* FIXME */, (RXB, TXB), Self> {
                // NOTE(unsafe) We own the buffer now and we won't call other `&mut` on it
                // until the end of the transfer.
                ...
                Transfer::r((rxbuffer, txbuffer), self)
            }
        }
    };
}
